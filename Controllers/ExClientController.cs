...
{
    [Authorize]
    public class ExClientController : Controller
    {
        private readonly ApplicationDbContext _context;

        public ExClientController(ApplicationDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> Index(string? searchString)
        {
            ...
            var clientsList = await query.Select(c => new ClientListViewModel
            {
                ...
                Contact_code_postal = c.PersonnesContact
                    .Select(p => p.Localite != null ? p.Localite.Code_postal : null).FirstOrDefault(),
                Contact_commune = c.PersonnesContact
                    .Select(p =>p.Localite !=null && p.Localite.CommuneBXL != null ? p.Localite.CommuneBXL.Commune_principale : null).FirstOrDefault(),
                Contact_pays = c.PersonnesContact
                    .Select(p => p.Localite != null && p.Localite.Pays != null ? p.Localite.Pays.Nom_pays : null).FirstOrDefault(),
        ...
        public async Task<IActionResult> Create(ClientCreateViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    // 4. Créez un site web principal (si le nom ou l'adresse est spécifié).
                    if (!string.IsNullOrWhiteSpace(model.Site_Nom_site) || !string.IsNullOrWhiteSpace(model.Site_Rue))
                    {
                        var site = new AdresseExploitation
                        {
                        ...
                        _context.AdressesExploitation.Add(site);
                    }
        ...
        //{
        //    model.LocaliteList = await _context.Localites
        ...
        //                ? u.Nickname : (u.Donnees_perso != null ? $"{u.Donnees_perso.Nom}{u.Donnees_perso.Prenom}" : u.Email)
        //        }).ToListAsync();
        //}
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    else if (!string.IsNullOrWhiteSpace(model.Siege_Raison_sociale) || !string.IsNullOrWhiteSpace(model.Siege_Adresse))
                    {
                        ...
                        client.SiegeSociale = newSiege;
                    }

                    // Обновление первичности контакта
                    var contact = client.PersonnesContact.FirstOrDefault();
                    if(!string.IsNullOrWhiteSpace(model.Contact_Nom))
                    {
                        if (contact != null)
        ...
        //        }).ToListAsync();
        //}
        // 5. SUPPRESSION LOGIQUE (SOFT DELETE)
